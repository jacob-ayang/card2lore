...
纯前端（Vite + TypeScript）页面：上传酒馆角色卡（`.json` / `.png`），提取 `character_book`，输出并下载两份 JSON：
- raw worldbook（原始结构）
...
1. 上传含 `data.character_book` 的 `.json` 角色卡，确认页面显示条目数，且可下载两份 JSON。
2. 上传含 `tEXt/chara` 的 `.png` 角色卡，确认同样可下载两份 JSON。
